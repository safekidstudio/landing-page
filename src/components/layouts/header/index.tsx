...
export default function SiteHeader({ settings }: SiteHeaderProps) {
    ...
    const { logo_image, navigation_items, login_button, cta_button } = settings.data;
                ...
                <div className="flex items-center gap-8 lg:gap-12">
                    ...
                    <nav className="hidden md:flex items-center gap-6 lg:gap-8">
                        {navigation_items.map((item, idx) => (
                            ...
                            >
                                {item.label === "*" ? (
                                    <span className="text-fuchsia-500 font-extrabold text-base scale-125 inline-block -mt-1">*</span>
                                ) : (
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="hidden md:flex items-center gap-5">
                        ...
                        {isFilled.keyText(cta_button.text) && (
                            <Button variant="brand" className="rounded-full px-5 py-2 h-auto font-medium" asChild>
                    ...
                    <Sheet>
                        ...
                        <SheetContent side="right" className="w-[300px] flex flex-col justify-between pt-16 bg-background">
                                ...
                                <nav className="flex flex-col space-y-4">
                                    {navigation_items.map((item, idx) => (
                                        <SheetClose asChild key={idx}>
                                            <PrismicNextLink
                                                field={item.link}
                                                className={cn("text-base font-medium text-muted-foreground hover:text-foreground transition-colors", {
                                                    "text-brand": isFilled.link(item.link) && item.link.url === pathname
                                                })}
                                            >
                                                {item.label === "*" ? (
                                                    <span className="text-fuchsia-500 font-extrabold text-base">*</span>
                                                ) : (
                                                    <span>{item.label}</span>
                                                )}
                                            </PrismicNextLink>
                            ...
                            <div className="border-t border-border pt-6 flex flex-col gap-4">
                                {isFilled.keyText(login_button.text) && (
                                    <SheetClose asChild>
                                        <PrismicNextLink
                                            field={login_button}
                                            className={cn(buttonVariants({ variant: "ghost" }), "w-full justify-center text-base font-medium")}
                                        />
                                    </SheetClose>
                                )}
                                {isFilled.keyText(cta_button.text) && (
                                    <SheetClose asChild>
                                        <Button variant="brand" className="rounded-full w-full py-2.5 h-auto text-base font-medium" asChild>
                                            <PrismicNextLink field={cta_button} />
                                        </Button>
                                    </SheetClose>
                                )}
